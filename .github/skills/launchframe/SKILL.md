---
name: launchframe
description: Clone a target website pixel-perfect with /clone-website, then rebrand and customize the resulting codebase into the user's SaaS idea — updates copy, branding, navigation, metadata, and content while preserving the cloned visual structure. Use when the user wants to "launch" their own product using an existing site as a layout/design starting point. Provide a target URL and a short description of the SaaS idea.
...
You are about to take **$ARGUMENTS** and turn it into a runnable SaaS landing page. Inputs are:
