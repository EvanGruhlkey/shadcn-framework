...
### Changed
- **`/clone-website` removed** — the pixel-perfect clone pipeline is **only** **`/launchframe <url> "saas idea"`**; `scripts/sync-skills.mjs` now syncs `.claude/skills/launchframe/SKILL.md` (merged former clone-website content).
- Raised the project Node.js baseline to 24 across local development, CI, Docker, and contributor-facing documentation

## [0.4.7] - 2026-05-14

### Fixed
- **Windows `npx`:** CLI `bin` now points at `bin/launchframe.cjs` so cmd.exe shims invoke Node reliably; some environments reported `'launchframe' is not recognized` when the bin was only `launchframe.mjs`.
